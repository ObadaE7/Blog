<?php

return [
    'navigation' => [
        'Dashboard' => 'Dashboard',
        'Profile' => 'Profile',
        'My Profile' => 'My Profile',
        'Home' => 'Home',
        'Settings' => 'Settings',
        'Logout' => 'Logout',
        'Roles' => 'Roles',
        'Countries' => 'Countries',
        'Categories' => 'Categories',
        'Tags' => 'Tags',
        'Users' => 'Users',
        'Analysis' => 'Analysis',
        'Articles' => 'Articles',
        'Create' => 'Create',
        'Show' => 'Show',
        'Statistics' => 'Statistics',
        'Basic tables' => 'Basic tables',
        'Secondary tables' => 'Secondary tables',
        'Site Settings' => 'Site Settings',
    ],

    'profile' => [
        'Personal information' => 'Personal information',
        'First name' => 'First name',
        'First name placeholder' => 'Enter your first name',
        'Last name' => 'Last name',
        'Last name placeholder' => 'Enter your last name',
        'Username' => 'Username',
        'User name placeholder' => 'Enter your user name',
        'Country' => 'Country',
        'Country placeholder' => 'Select your country',
        'Role' => 'Role',
        'Role placeholder' => 'Select your role',
        'Saves automatically' => 'The form saves automatically when you click outside the field.',
        'About me' => 'About me',
        'Bio' => 'Bio',
        'Bio placeholder' => 'Tell me something about you',
        'Phone' => 'Phone',
        'Phone placeholder' => 'Enter your phone number',
        'Birthday' => 'Birthday',
        'Update email' => 'Update email',
        'Email' => 'Email',
        'Email placeholder' => 'Enter your email',
        'Current password' => 'Current password',
        'Current password placeholder' => 'Enter your current password',
        'New password' => 'New password',
        'New password placeholder' => 'Enter your new password',
        'Confirm password' => 'Confirm password',
        'Confirm password placeholder' => 'Confirm your password',
        'Update password' => 'Update password',
        'Save' => 'Save',
        'Change cover' => 'Change cover',
        'Temp cover' => 'Temp cover',
        'Profile cover' => 'Profile cover',
        'Change avatar' => 'Change avatar',
        'Temp avatar' => 'Temp avatar',
        'Profile avatar' => 'Profile avatar',
        'Delete account' => 'Delete account',
        'Delete account warning' => 'If you delete your account, you won\'t be able to recover it later. Please confirm before continuing.',
        'Delete my account' => 'Delete my account',
        'Confirm delete' => 'Confirm deletion of your account',
        'Confirm delete account warning' => 'Please note that once you delete your account, this action cannot be undone. All your data will be lost and you will not be able to access your account again. If you are sure you want to delete your account, please enter your current password to confirm the process.',
    ],

    'analysis' => [
        'Total articles' => 'Total articles',
        'Article' => 'Article',
        'Total reactions' => 'Total reactions',
        'Reaction' => 'Reaction',
        'Reaction rate' => 'Reaction rate',
        'Reaction article' => 'Reaction for each article',
        'Articles highest' => 'Articles with the highest reaction rate',
        'Track posting' => 'Track daily posting status',
        'Number of likes' => 'Number of likes',
        'Number of dislikes' => 'Number of dislikes',
        'Number of articles' => 'Number of articles',
        'Reactions' => 'Reactions',
        'Empty line one' => 'Sorry, there is no data to display at the moment',
        'Empty line two' => 'what are you waiting for? Publish your first articles!',
    ],

    'modal' => [
        'Close' => 'Close',
        'Delete' => 'Delete',
        'Update' => 'Update',
        'Create' => 'Create',

        'roles' => [
            'Create role title' => 'Create a new role',
            'Name' => 'Name',
            'Name placeholder' => 'Enter the role name',
            'Description' => 'Description',
            'Description placeholder' => 'Describe this role',
            'Delete role title' => 'Delete role',
            'Delete role warning' => 'Are you sure you want to delete this role?',
            'Edit role title' => 'Edit role',
        ],

        'countries' => [
            'Create country title' => 'Create a new country',
            'Name' => 'Name',
            'Name placeholder' => 'Enter the country name',
            'Flag' => 'Flag',
            'Delete country title' => 'Delete country',
            'Delete country warning' => 'Are you sure you want to delete this country?',
            'Edit country title' => 'Edit country',
            'Click here' => 'Click here',
        ],

        'categories' => [
            'Create category title' => 'Create a new category',
            'Name' => 'Name',
            'Name placeholder' => 'Enter the category name',
            'Slug' => 'Slug',
            'Slug placeholder' => 'Enter the slug',
            'Description' => 'Description',
            'Description placeholder' => 'Describe this category',
            'Thumbnail' => 'Thumbnail',
            'Click here' => 'Click here',
            'Delete category title' => 'Delete category',
            'Delete category warning' => 'Are you sure you want to delete this category?',
            'Edit category title' => 'Edit category',
        ],

        'tags' => [
            'Create tag title' => 'Create a new tag',
            'Name' => 'Name',
            'Name placeholder' => 'Enter the tag name',
            'Slug' => 'Slug',
            'Slug placeholder' => 'Enter the slug',
            'Delete tag title' => 'Delete tag',
            'Delete tag warning' => 'Are you sure you want to delete this tag?',
            'Edit tag title' => 'Edit tag',
        ],

        'posts' => [
            'Create tag title' => 'Create a new tag',
            'Name' => 'Name',
            'Name placeholder' => 'Enter the tag name',
            'Slug' => 'Slug',
            'Slug placeholder' => 'Enter the slug',
            'Delete post title' => 'Delete post',
            'Delete post warning' => 'Are you sure you want to delete this post?',
            'Edit tag title' => 'Edit tag',
        ],
    ],

    'post' => [
        'Image placeholder' => 'Click here to upload image',
        'Title' => 'Title',
        'Title placeholder' => 'Enter the article title',
        'Subtitle' => 'Subtitle',
        'Subtitle placeholder' => 'Enter the article subtitle',
        'Slug' => 'Slug',
        'Slug placeholder' => 'Enter-the-article-slug',
        'Content' => 'Content',
        'Content placeholder' => 'Enter the article content',
        'Category' => 'Category',
        'Category placeholder' => 'Choose a category',
        'Status' => 'Status',
        'Draft' => 'Draft',
        'Published' => 'Published',
        'Tags' => 'Tags',
        'Tags placeholder' => 'Choose tags',
        'Create' => 'Create',
        'Articles' => 'Articles',
        'Show' => 'Show',
        'Search here' => 'Search here',
        'Search by' => 'Search by',
        'Per page' => 'Per page',
        'Newest' => 'Newest',
        'Oldest' => 'Oldest',
        'Quick peek' => 'Quick peek',
        'Edit' => 'Edit',
        'Delete' => 'Delete',
        'Created at' => 'Created at',
    ],

    'notification' => [
        'Mark all' => 'Mark all as read',
        'Delete all' => 'Delete all',
        'View article' => 'View article',
        'Mark' => 'Mark as read',
        'Delete' => 'Delete',
        'Empty notification' => 'No notification',
        'View all' => 'View all notifications',
        'Liked' => 'He liked your article',
        'Disliked' => 'Didn\'t like your article',
        'Interact' => 'Interact with your article',
        'Commented' => 'He commented on your article',
    ],

    'settings' => [
        'General' => 'General',
        'General title' => 'General Settings',
        'General subtitle' => 'You can manage key aspects of your site, including title, description, icon, logos, and language preferences. These settings determine the basic look and identity of your site.',
        'SiteName' => 'Site name',
        'SiteName placeholder' => 'Enter the site name',
        'SiteLanguage' => 'Site language',
        'SiteLanguage placeholder' => 'Choose the site language',
        'SiteDescription' => 'Site description',
        'SiteDescription placeholder' => 'Enter the site description',
        'Keywords' => 'Keywords',
        'Keywords placeholder' => 'Enter the site keywords',
        'Email' => 'Email',
        'Email placeholder' => 'Enter the email',
        'Phone' => 'Phone',
        'Phone placeholder' => 'Enter the phone number',
        'Address' => 'Address',
        'Address placeholder' => 'Enter the address',
        'Favicon' => 'Favicon',
        'LabelText' => 'Click here to upload',
        'SiteLogo' => 'Site logo',
        'SiteLightLogo' => 'Light theme',
        'SiteDarkLogo' => 'Dark theme',
        'Social' => 'Social',
        'Facebook' => 'Facebook url',
        'Facebook placeholder' => 'Enter facebook url',
        'Instagram' => 'Instagram url',
        'Instagram placeholder' => 'Enter instagram url',
        'Twitter' => 'Twitter url',
        'Twitter placeholder' => 'Enter twitter url',
        'Youtube' => 'Youtube url',
        'Youtube placeholder' => 'Enter youtube url',
        'Social title' => 'Social media settings',
        'Social subtitle' => 'Update your social media links below',
    ],

    'table' => [
        'Id' => 'Id',
        'Name' => 'Name',
        'Description' => 'Description',
        'Actions' => 'Actions',
        'No results found' => 'No Results Found',
        'Search here' => 'Search here ...',
        'Search by' => 'Search by',
        'Per page' => 'Per page',
        'Options' => 'Options',
        'Create' => 'Create',
        'Refresh' => 'Refresh',
        'Reset' => 'Reset',
        'Export' => 'Export',
        'Pdf' => 'Pdf',
        'Excel' => 'Excel',
        'Csv' => 'Csv',
        'Print' => 'Print',
        'Table' => 'Table',
        'Roles' => 'Roles',
        'Countries' => 'Countries',
        'Flag' => 'Flag',
        'Categories' => 'Categories',
        'Image' => 'Image',
        'Slug' => 'Slug',
        'Articles' => 'Articles',
        'Tags' => 'Tags',
        'Avatar' => 'Avatar',
        'Email' => 'Email',
        'Users' => 'Users',
        'Title' => 'Title',
        'Subtitle' => 'Subtitle',
        'Category' => 'Category',
        'Status' => 'Status',
        'Fname' => 'First name',
        'Lname' => 'Last name',
        'Uname' => 'User name',
        'Articles_count' => 'Articles_count',
    ],
];
