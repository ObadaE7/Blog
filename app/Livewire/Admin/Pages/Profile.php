...
class Profile extends Component
{
    ...
    public function savePassword()
    {
        ...
        try {
            if (Hash::check($this->current_password, $admin->password)) {
                Admin::findOrFail($admin->id)->update(['password' => Hash::make($validated['password'])]);
            }
    ...
    public function updatedAvatar()
    {
        ...
        try {
            $existingAvatar = $admin->imageable()->where('type', 'avatar')->first();
            if ($existingAvatar) {
                Storage::disk('public')->delete($existingAvatar->url);
                $existingAvatar->delete();
            }
            $avatarPath = $validated['avatar']->store('avatars', 'public');
            $admin->imageable()->create(['url' => $avatarPath, 'type' => 'avatar']);
    ...
    public function deleteAccount()
    {
        ...
        $this->validate(['current_password_account' => 'required|string|current_password:admin']);

        try {
            if (Hash::check($this->current_password_account, $admin->password)) {
