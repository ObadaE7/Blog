...
class CountryTable extends Component
{
    ...
    public function create()
    {
        ...
        try {
            $flagPath = $validated['flag']->store('flags', 'public');
            $country = Country::create($validated);
            $country->imageable()->create(['url' => $flagPath, 'type' => 'flag']);
    ...
    public function edit($id)
    {
        ...
        $this->existingFlag = $country->imageable->url ?? '';
    }
    ...
    public function update($id)
    {
        ...
        try {
            $existingFlag = $country->imageable()->where('type', 'flag')->first();

            if ($existingFlag) {
                if ($country->imageable) {
                    Storage::disk('public')->delete($existingFlag->url);
                    $existingFlag->delete();
                }
                $flagPath = $validated['flag']->store('flags', 'public');
            } else {
                $flagPath = $this->existingFlag;
            }
    ...
    public function render()
    {
        ...
        $headers = [
            trans('dashboard.table.Id'),
            trans('dashboard.table.Flag'),
            trans('dashboard.table.Name'),
            trans('dashboard.table.Actions')
        ];
        $perPages = [5, 10, 20, 50];
